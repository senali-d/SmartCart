...
const Card = ({ id, image, name, rating, price, size, category, ingredients, nutritions }: CardProps) => {
      ...
      <div className="w-[99%] bg-white border border-gray-200 rounded-lg shadow mt-5">
          ...
          </Link>
        {
            ...
            <div className="flex px-2 flex-col">
              <div className="w-full">
                ...
                <ul className="ml-8 text-gray-700">
                  {
                    ingredients?.map(ingredient => <li className="list-disc" key={ingredient}>{ingredient}</li>)
                  }
                </ul>
              </div>
              <div className="w-full">
                ...
                <ul className="ml-8 text-gray-700">
                  {
                    nutritions?.map(nutrition => <li className="list-disc" key={nutrition.name}>
          ...
          <div className="flex items-center mt-2.5 mb-5">
            {stars.map(star => <AiFillStar className="text-yellow-500" size={20} key={star} />)}
            {emptyStars.map(star => <AiOutlineStar className="text-yellow-500" size={20} key={star} />)}
